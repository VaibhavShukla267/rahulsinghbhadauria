...
function OrganizationAdminPage() {
    ...
    const handleSelectChange = (name, value) => {
        ...
        if (name === 'vidansabha') {
            ...
            if (value === siteConfig.specialConstituency) newState.areaType = 'urban';
        }
    ...
    return (
        <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="mb-8 p-4 border rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                ...
                {formData.vidansabha && formData.vidansabha !== siteConfig.specialConstituency && ( <div className="flex items-center space-x-4"> <Label>Area Type:</Label> <RadioGroup value={formData.areaType} onValueChange={handleAreaTypeChange} className="flex gap-4"> <div className="flex items-center space-x-2"><RadioGroupItem value="rural" id="rural" /><Label htmlFor="rural">Rural</Label></div> {showUrbanOption && (<div className="flex items-center space-x-2"><RadioGroupItem value="urban" id="urban" /><Label htmlFor="urban">Urban</Label></div>)} </RadioGroup> </div> )}
                ...
                {formData.vidansabha === siteConfig.specialConstituency && ( <Select onValueChange={(value) => handleSelectChange('ward', value)} value={formData.ward}> <SelectTrigger><SelectValue placeholder="वार्ड का चयन करें" /></SelectTrigger> <SelectContent>{siteConfig.specialWards.map(ward => <SelectItem key={ward} value={ward}>{ward}</SelectItem>)}</SelectContent> </Select> )}
                {formData.areaType === 'urban' && formData.vidansabha !== siteConfig.specialConstituency && ( <Select onValueChange={(value) => handleSelectChange('ward', value)} value={formData.ward}> <SelectTrigger><SelectValue placeholder="वार्ड का चयन करें" /></SelectTrigger> <SelectContent>{districts[formData.jila].nagar_nikay.ward.map(ward => <SelectItem key={ward} value={ward}>{ward}</SelectItem>)}</SelectContent> </Select> )}
